...
const (
	BitDate         = 1 << iota                            //1 << 0 which is 00000001 日期标记位  2019/01/23
	BitTime                                                //1 << 1 which is 00000010 时间标记位  01:23:12
	BitMicroSeconds                                        //1 << 2 which is 00000100 微秒级标记位 01:23:12.111222
	BitLongFile                                            //1 << 3 which is 00001000 完整文件名称 /home/go/src/zinx/server.go
	BitShortFile                                           //1 << 4 which is 00010000 最后文件名   server.go
...
func NewZinxLog(out io.Writer, prefix string, flag int) *ZinxLogger {
	...
	//https://zhuanlan.zhihu.com/p/76504936 [主动触发垃圾回收]
...
/*
制作当条日志数据的 格式头信息。 作者这种按位条件判断方法，很不错，比起传统方法更加清晰易懂
bytes.Buffer 类型
