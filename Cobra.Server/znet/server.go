...
func NewServer(opts ...Option) ziface.IServer {
	...
	println(zutils.GlobalObject.Name)
...
func (s *Server) Start() {
	...
	s.exitChan = make(chan struct{}) // 但在并发编程中，channel之间的通讯，可以使用一个struct{}作为信号量。 用来接收关闭信号
	...
	go func() {
		...
		go func() {
			...
			for {
				...
				//4.2 设置服务器最大连接控制，如果超过最大连接，那么则关闭此新的连接
				if s.ConnMgr.Len() >= zutils.GlobalObject.MaxConn {
					conn.Close()
					continue
				}

				//4.3 处理该新连接请求的 业务 方法，此时应该有handler 和 conn是绑定的
...
func (s *Server) Serve() {
	...
	//阻塞,否则主Go退出， listenner的go将会退出。【这是select常用的一种场景】
	select {}
}
